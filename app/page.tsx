      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            ...
            <p className="mt-6 max-w-3xl text-xl text-zinc-300">Maszyny i narzędzia dostępne w sprzedaży przez Allegro, ERLI oraz bezpośrednio w TrendEco.</p>
            <p className="mt-4 max-w-3xl text-base text-zinc-400">Zamówienie za pobraniem ustalamy przez WhatsApp. Koszt dostawy potwierdzamy przed wysyłką.</p>
          </div>
          <div className="flex flex-wrap gap-3">
            <a href={tiktokLink} target="_blank" rel="noopener noreferrer" className="rounded-full border border-white/20 bg-white/10 px-5 py-3 font-bold transition hover:bg-white/20">TikTok TrendEco</a>
            <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="rounded-full bg-green-600 px-5 py-3 font-bold text-white transition hover:bg-green-500">Zadzwoń / WhatsApp +48 512 077 770</a>
          </div>
        </div>
                  ...
                  return (
                    <article key={product.id} className="rounded-3xl border border-zinc-200 bg-zinc-50 p-4 transition hover:-translate-y-1 hover:shadow-lg">
                      <a href={getOfferPath(product)}>
                        <div className="flex h-48 items-center justify-center overflow-hidden rounded-2xl bg-white">{product.image ? <img src={product.image} alt={product.name} className="max-h-full max-w-full object-contain" /> : <span className="text-zinc-400">Brak zdjęcia</span>}</div>
                      ...
                      <div className="mt-4 grid gap-2">
                        ...
                        <a href={getOfferPath(product)} className="rounded-full bg-orange-500 px-4 py-3 text-center text-sm font-bold text-white">Kup na Allegro</a>
                      </div>
                    </article>
                  );
                })}

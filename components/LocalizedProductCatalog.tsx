      ...
      <div className="mx-auto max-w-7xl">
        ...
        {sections.map((section) => {
          ...
          return <section key={section} id={categoryId(section)} className="scroll-mt-6 pb-10"><h2 className="mb-4 border-b border-zinc-200 pb-3 text-2xl font-black sm:text-4xl">{t.categories[section]}</h2><div className="grid grid-cols-2 gap-3 sm:gap-6 xl:grid-cols-3">{sectionProducts.map((product) => {
            ...
            const whatsappHref = `https://wa.me/48512077770?text=${encodeURIComponent(t.whatsappOrder(product.name, product.id))}`;
            return <article key={product.id} className="flex min-w-0 flex-col rounded-2xl border border-zinc-200 bg-zinc-50 p-2.5 sm:rounded-3xl sm:p-4"><a href={href} target="_blank" rel="noopener noreferrer sponsored" className="block min-w-0"><div className="flex aspect-square items-center justify-center overflow-hidden rounded-xl bg-white p-1 sm:h-48 sm:aspect-auto sm:rounded-2xl sm:p-0">{product.image ? <img src={product.image} alt={product.name} loading="lazy" className="max-h-full max-w-full object-contain" /> : null}</div><h3 className="mt-2 line-clamp-3 min-h-[3.6rem] break-words text-[13px] font-bold leading-[1.2] sm:mt-4 sm:text-base sm:leading-normal">{product.name}</h3></a><p className="mt-2 text-[11px] text-zinc-600 sm:text-sm">{product.stock > 0 ? `${t.available}: ${product.stock}` : t.unavailable}</p><div className="mt-auto grid gap-2 pt-3"><a href={whatsappHref} target="_blank" rel="noopener noreferrer" className="flex min-h-11 items-center justify-center rounded-xl bg-green-600 px-2 py-2 text-center text-[11px] font-black text-white transition hover:bg-green-500 active:scale-[0.98] sm:rounded-full sm:px-4 sm:py-3 sm:text-sm">{t.buyCourier}</a><a href={href} target="_blank" rel="noopener noreferrer sponsored" className="flex min-h-11 items-center justify-center rounded-xl bg-orange-500 px-2 py-2 text-center text-[11px] font-black text-white active:scale-[0.98] sm:rounded-full sm:px-4 sm:py-3 sm:text-sm">{t.buyAllegro}</a></div></article>;
          })}</div></section>;
        })}

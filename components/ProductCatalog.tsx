        ...
        <div className="sticky top-0 z-20 -mx-3 mb-5 border-b border-zinc-200 bg-white/95 px-3 pb-3 pt-3 backdrop-blur sm:static sm:mx-0 sm:mb-8 sm:border-0 sm:bg-transparent sm:px-0 sm:pb-0 sm:pt-0 sm:backdrop-blur-none">
          ...
          <nav className="mt-3 flex gap-2 overflow-x-auto pb-1 [-webkit-overflow-scrolling:touch] lg:hidden">
            {sections.map((section) => (
              <a key={section} href={`#${categoryId(section)}`} className="flex min-h-11 min-w-max items-center gap-2 rounded-full border border-zinc-200 bg-zinc-50 px-4 text-sm font-bold active:scale-[0.98]">
                <span>{section}</span><span className="rounded-full bg-zinc-200 px-2 py-0.5 text-xs text-zinc-600">{counts[section]}</span>
              </a>
            ))}
          </nav>
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-6">
              ...
              <nav className="flex flex-col gap-2">
                {sections.map((section) => (
                  <a key={section} href={`#${categoryId(section)}`} className="flex w-full items-center justify-between gap-3 rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm font-bold transition hover:border-orange-400 hover:bg-orange-50 hover:text-orange-700">
                    <span>{section}</span><span className="rounded-full bg-zinc-200 px-2 py-0.5 text-xs text-zinc-600">{counts[section]}</span>
                  </a>
                ))}
              </nav>
          ...
          <div className="min-w-0">
            {sections.map((section) => {
              const sectionProducts = filteredProducts
                .filter((product) => product.category === section)
                .sort((a, b) => section === "Budownictwo" ? constructionRank(a) - constructionRank(b) : 0);
                        ...
                        <article key={product.id} className="flex min-w-0 flex-col rounded-2xl border border-zinc-200 bg-zinc-50 p-2.5 sm:rounded-3xl sm:p-4 sm:transition sm:hover:-translate-y-1 sm:hover:shadow-lg">
                          <a href={getOfferPath(product)} className="block min-w-0">
                            <div className="flex aspect-square items-center justify-center overflow-hidden rounded-xl bg-white p-1 sm:h-48 sm:aspect-auto sm:rounded-2xl sm:p-0">
                              {product.image ? <img src={product.image} alt={product.name} loading="lazy" className="max-h-full max-w-full object-contain" /> : <span className="text-xs text-zinc-400 sm:text-base">Brak zdjęcia</span>}
                            </div>
            ...
            {filteredProducts.length === 0 && (
              <div className="rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 px-5 py-12 text-center sm:rounded-3xl sm:px-6 sm:py-16">
                ...
                <button type="button" onClick={() => setQuery("")} className="mt-5 min-h-11 rounded-full bg-zinc-900 px-5 py-3 text-sm font-bold text-white">Wyczyść wyszukiwanie</button>
              </div>
            )}
          </div>
        </div>

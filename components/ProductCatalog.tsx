...
function getCashOnDeliveryLink(product: AllegroProduct) {
  ...
  const message = ["Dzień dobry, chcę zamówić za pobraniem:", product.name, `Cena TrendEco: ${discountedPrice} ${product.currency}`, `Allegro ID: ${product.id}`, "Proszę o potwierdzenie dostępności oraz kosztu dostawy."].join("\n");
        ...
        <div className="sticky top-0 z-20 -mx-3 mb-5 border-b border-zinc-200 bg-white/95 px-3 pb-3 pt-3 backdrop-blur sm:static sm:mx-0 sm:mb-8 sm:border-0 sm:bg-transparent sm:px-0 sm:pb-0 sm:pt-0 sm:backdrop-blur-none">
          ...
          <nav className="mt-3 grid grid-cols-2 gap-2 lg:hidden">
            {sections.map((section) => (
              <a key={section} href={`#${categoryId(section)}`} className="flex min-h-11 items-center justify-center rounded-xl border border-zinc-200 bg-zinc-50 px-3 text-center text-sm font-bold active:scale-[0.98]">{section}</a>
            ))}
          </nav>
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-6">
              ...
              <nav className="grid grid-cols-2 gap-2">
                {sections.map((section) => <a key={section} href={`#${categoryId(section)}`} className="flex min-h-12 items-center justify-center rounded-xl border border-zinc-200 bg-zinc-50 px-2 text-center text-xs font-bold transition hover:border-orange-400 hover:bg-orange-50 hover:text-orange-700">{section}</a>)}
              </nav>
            </div>
          </aside>
            ...
            {sections.map((section) => {
              ...
              return (
                <section key={section} id={categoryId(section)} className="scroll-mt-32 pb-9 last:pb-0 sm:scroll-mt-6 sm:pb-14">
                  <div className="mb-4 border-b border-zinc-200 pb-3 sm:mb-6 sm:pb-4"><h2 className="text-2xl font-black sm:text-4xl">{section}</h2></div>
                      ...
                      return (
                        <article key={product.id} className="flex min-w-0 flex-col rounded-2xl border border-zinc-200 bg-zinc-50 p-2.5 sm:rounded-3xl sm:p-4 sm:transition sm:hover:-translate-y-1 sm:hover:shadow-lg">
                          <a href={getOfferPath(product)} className="block min-w-0">
                            <div className="flex aspect-square items-center justify-center overflow-hidden rounded-xl bg-white p-1 sm:h-48 sm:aspect-auto sm:rounded-2xl sm:p-0">{product.image ? <img src={product.image} alt={product.name} loading="lazy" className="max-h-full max-w-full object-contain" /> : <span className="text-xs text-zinc-400 sm:text-base">Brak zdjęcia</span>}</div>
                          ...
                          <div className="mt-2 sm:mt-3"><p className="text-[11px] leading-tight text-zinc-400 line-through sm:text-sm">Allegro: {product.price} {product.currency}</p>{erli && <p className="text-[11px] leading-tight text-zinc-400 line-through sm:text-sm">ERLI: {erli.price} {erli.currency}</p>}<p className="mt-1 text-base font-black leading-tight text-green-700 sm:text-xl">{discountedPrice} {product.currency}</p><p className="text-[11px] font-bold text-green-700 sm:text-sm">Cena TrendEco</p></div>
                          ...
                          <div className="mt-auto grid gap-1.5 pt-3 sm:gap-2 sm:pt-4">
                            <a href={getCashOnDeliveryLink(product)} target="_blank" rel="noopener noreferrer" className="flex min-h-11 items-center justify-center rounded-xl bg-green-600 px-2 py-2 text-center text-[11px] font-black leading-tight text-white active:scale-[0.98] sm:rounded-full sm:px-4 sm:py-3 sm:text-sm">Zamów za pobraniem</a>
                            {erli && <a href={erli.url} target="_blank" rel="noopener noreferrer sponsored" className="flex min-h-10 items-center justify-center rounded-xl bg-[#00B8B0] px-2 py-2 text-center text-[11px] font-black text-white active:scale-[0.98] sm:rounded-full sm:px-4 sm:py-3 sm:text-sm">Kup na ERLI</a>}
                            <a href={getOfferPath(product)} className="flex min-h-10 items-center justify-center rounded-xl bg-orange-500 px-2 py-2 text-center text-[11px] font-bold text-white active:scale-[0.98] sm:rounded-full sm:px-4 sm:py-3 sm:text-sm">Kup na Allegro</a>
                          </div>
                        </article>
                      );
                    })}
